...
const statisticsAttributes = [
  { value: 16, label: '系统内存使用率' },
  { value: 18, label: '系统FD使用情况' },
  { value: 19, label: '系统网络连接数' },
  { value: 20, label: '发送/接收字节数' },
  { value: 33, label: '进程内存使用情况' },
  { value: 34, label: '进程FD使用情况' },
  { value: 48, label: '关键目录空间使用率' },
  { value: 64, label: '网络延迟' },
  { value: 65, label: '丢包率' },
  { value: 80, label: '雷达数据完整率' },
  { value: 81, label: '雷视数据实时率' },
  { value: 82, label: '雷达数据跟踪率' }
]
...
const fetchQueryData = async (beginTime, endTime, queryType) => {
  try {
    const res = await axios.post('/MonitorData', {
      beginTime,
      endTime,
      queryType
    })
