...
const statisticsAttributes = [
  ...
  { value: 517, label: '最大排队长度' },
  { value: 518, label: '最大排队车辆数' },
  { value: 519, label: '车道空闲率' },
  { value: 520, label: '车道饱和率' },
  { value: 521, label: '最大红初车辆数' },
  { value: 522, label: '绿灯损失率' },
  { value: 523, label: '绿信比' }
]
...
const fetchQueryData = async (beginTime, endTime, queryType) => {
  try {
    const res = await axios.post('/TrafficStaticQuery', {
      beginTime,
      endTime,
      queryType
    })
